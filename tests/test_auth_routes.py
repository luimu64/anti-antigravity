...
async def test_auth_login_remote_html():
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://192.168.1.50:8000") as ac:
...
async def test_auth_callback_success():
    transport = httpx.ASGITransport(app=app)
    with patch.object(auth_manager, "exchange_code", new_callable=AsyncMock, return_value={"access_token": "ya29.test", "refresh_token": "1//test"}), \
         patch.object(client, "load_code_assist", new_callable=AsyncMock, return_value={"cloudaicompanionProject": "test-proj"}):
        async with httpx.AsyncClient(transport=transport, base_url="http://localhost:8000") as ac:
            resp = await ac.get("/auth/callback?code=test_code_123&state=test_state_456")
            assert resp.status_code == 200
...
async def test_auth_callback_exchange_exception():
    transport = httpx.ASGITransport(app=app)
    with patch.object(auth_manager, "exchange_code", new_callable=AsyncMock, side_effect=ValueError("Token exchange network error")):
        async with httpx.AsyncClient(transport=transport, base_url="http://localhost:8000") as ac:
...
async def test_auth_exchange_direct_refresh_or_access_token():
    transport = httpx.ASGITransport(app=app)
    with patch.object(client, "load_code_assist", new_callable=AsyncMock, return_value={}):
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
            # 1. Refresh token starting with 1//
            resp_ref = await ac.post("/auth/exchange", json={"code_or_url": "1//test_refresh_token_abc"})
            ...
            resp_acc = await ac.post("/auth/exchange", json={"code_or_url": "ya29.test_access_token_xyz"})
...
async def test_auth_exchange_full_url_and_code():
    transport = httpx.ASGITransport(app=app)
    with patch.object(auth_manager, "exchange_code", new_callable=AsyncMock, return_value={"access_token": "ya29.exchanged"}), \
         patch.object(client, "load_code_assist", new_callable=AsyncMock, return_value={}):
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
            # Full URL
            ...
            # Plain code
            resp2 = await ac.post("/auth/exchange", json={"code_or_url": "4/0AeanS0...", "state": "teststate123"})
...
async def test_auth_exchange_failure():
    transport = httpx.ASGITransport(app=app)
    with patch.object(auth_manager, "exchange_code", new_callable=AsyncMock, side_effect=ValueError("Invalid code")):
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
            resp = await ac.post("/auth/exchange", json={"code_or_url": "invalid_code_here"})
            assert resp.status_code == 400
            assert "Authorization code exchange failed" in resp.json()["error"]["message"]
...
async def test_auth_token_set_and_refresh():
    ...
    with patch.object(auth_manager, "refresh_access_token", new_callable=AsyncMock, return_value="ya29.new_refreshed_access_token"), \
         patch.object(client, "load_code_assist", new_callable=AsyncMock, return_value={}):
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
    ...
    with patch.object(auth_manager, "refresh_access_token", new_callable=AsyncMock, side_effect=ValueError("Token revoked")):
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
