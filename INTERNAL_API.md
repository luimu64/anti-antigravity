...
- **Authentication**:
  ...
  - The page also runs a Web Anti-Abuse handshake (`waa-pa.clients6.google.com/$rpc/google.internal.waa.v1.Waa/Create`) at load time; its token is not carried in GenerateContent bodies or headers. The opaque client-context blob at payload slot `[4]` embeds a client-computed device attestation (a stable fingerprint segment is visible across sessions of the same browser). Requests whose blob fails the authorization stage receive `403 PERMISSION_DENIED` even though authentication succeeded - the gateway retries automatically with simpler slot-4 values (configured/synthetic blob -> `null` -> empty string) and pins whichever mode passes (`BLOB_MODES`).
